...
- ### BEAM_leakage.py: 
    Generates a plot of leakage level (error^2) as a function of error amplitude for the fixed $k_{in}=15$ and $k_{out}=25$ and after noise normalization. (data used in **Fig.14** created here and plotted in thesis_plot.ipynb.)

- ### BEAM_LeakVSk.py:
    Fixes the error amplitude (noise level in k space), and plots leakage level as a function of $k_{in}$. Also generates a csv file LeakvsK_{}{}.csv'.format(option,amp). (**Fig.10** generated in Beam_norm0403.ipynb.)

- ### r_calc: 
    A directory containing calculation of bias in r.

    - camb_96896687_totcls.dat.txt: CMB power spectrum downloaded from CAMB webpage
    ...
    - plot_r.py: Reads in the output .csv file from RnNoise.py and plots $\Delta r$ as a function of noise; The data file is also read in thesis_plot.ipynb which plots **Fig.16**.
...
- ### BEAM_leakage_filterk1.py:
    Calculates the leakage level as a function of normalized noise in $k$ space, by scanning the inner radius of the annular noise filter $k_{in}$ from 1 to ~70 and fixing width $dk=2$. Generates a csv file "data_k_r2.csv", which is read and plotted in the notebook ErrorMap_norm.ipynb and Beam_norm0403.ipynb (**Fig.9**)
...
- ### Beam_norm0403.ipynb:
    ...
    Contains **Fig.6**, **Fig.8**, **Fig.9**, **Fig.10**, and **Fig.11**. 

    - **Fig.9** uses data generated by BEAM_leakage_filterk1.py

    - **Fig.10** uses data generated by BEAM_LeakVSk.py

    - **Fig.11** uses data generated by BEAM_leakVSk_dk.py
...
- ### BEAM_leakVSk_RMSfix.py:
    By fixing RMS level in real space, this script calculates the leakage level as a function of $k_{in}$, and outputs to RMS{}_leak_k_{}.csv, including $k_{in}$, leakage level, scaling factors used for normalization, and RMS, which is read and plotted in Leak_k_fixRMS.ipynb (contraining **Fig.12**)
...
- ### Leak_k_fixRMS.ipynb:
    ...
    - **Fig.12** uses data generated by BEAM_leakVSk_RMSfix.py

- ### PowerSpectrum.ipynb:
    Generates leakage power spectra with measured $\Delta r$ labeled. (**Fig.15**)

- ### thesis_plot.ipynb:
    Generates some figures used in my thesis (by order): **Fig.13**, **Fig.17**, **Fig.4**, **Fig.16**, **Fig.14**. 

    - **Fig.16** uses data generated in ./r_calc/RnNoise.py

    - **Fig.14** uses data generated in BEAM_leakage.py
