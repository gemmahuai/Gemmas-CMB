...
if (len(sys.argv)!=9):
    ...
    # ex. python BEAM_leakage_filterk1.py 4096 1024 10.0 1.0 3.0 y amp 2.0
    sys.exit()
# input parameters
...
for j in range(len(k_in)):
    ...
    for i in range(len(amp)):
        ...
        #error[j,i] = np.mean(beam_diff_rela[1:3]) ## maxdeg=3.0
        #error[j,i] = np.mean(beam_diff_rela[4]) ## maxdeg=4.0
        error[j,i] = np.mean(beam_diff_rela[l_vec<500]) # average the beam difference over 0<l<500
