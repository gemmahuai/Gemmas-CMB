### 04/03/2023 updated
### this python script fixes the error amplitude (noise level in k space), and plots leakage level as a function of k_in
...
if (len(sys.argv)!=9):
    print('Wrong inputs!')
    print('Usage: python BEAM_LeakVSk.py N_screen N_theta_interp screenD sigma angle(deg) truncation(y/n) "phase/amp" amplitude')
    # ex. python BEAM_LeakVSk.py 4096 1024 10.0 1.0 5.0 y amp 0.1
    sys.exit()
# input parameters
...
error = np.zeros(len(k_in)) # beam window difference (leakage = error^2)
noise_norm = np.zeros_like(error) # normalized noise level in k space
...
### write to a csv file
with open('./data/LeakvsK_{}{}.csv'.format(option,amp), 'w') as f:
   writer = csv.writer(f, delimiter='\t')
   writer.writerows(zip(k_in, error**2))

### plot the leakage level as a function of inner radius k_in
