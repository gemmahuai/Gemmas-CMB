...
for i in range(len(amp)):
    ...
    error = np.append(error, np.mean(beam_diff_rela[(l_vec>=50)&(l_vec<=250)])) # average the beam difference at l=85 and l=170 (2nd and 3rd elements)
...
with open('./data/Replot_leakrms_{}.csv'.format(option), 'w') as f:
...
#plt.show()
plt.grid()
plt.savefig('./leak_rms_{}.png'.format(option))
