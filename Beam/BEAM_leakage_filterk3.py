### 02/15/2023 updated
### this python script compares the power spectrum of the perturbed and unperturbed beams with fixed k_in and increasing k_out
...
if (len(sys.argv)!=9):
    print('Wrong inputs!')
    print('Usage: python BEAM_leakage_filterk.py N_screen N_theta_interp screenD sigma angle(deg) truncation(y/n) phase/amp inner_radius')
    # ex. python BEAM_leakage_filterk3.py 4096 1024 10.0 1.0 3.0 y amp 5.0
    sys.exit()
# input parameters
...
kin = float(sys.argv[8])

amp = np.array([0.1, 0.3, 0.6, 1.0, 1.5, 2.0, 3.0, 5.0, 7.0, 10.0, 15.0, 20.0])
#amp = np.array([1.0, 2.0]) # test
radius = np.array([1,2,4,6,8,10,15,20,30,40,50]) # radius of the annulus -> k_out[i] = k_in + radius[i]]
error = np.zeros((len(radius),len(amp))) #leakage = error^2
...
amp_output = np.tile(amp, len(radius))
error_output = (error**2).flatten()
noise_output = noise_norm.flatten()
with open('./data_k_kin{}.csv'.format(kin), 'w') as f:
   writer = csv.writer(f, delimiter='\t')
   writer.writerows(zip(radius_output, amp_output, noise_output, error_output))
